...
from Models.RAFT.utils import flow_viz
...
def run(args, input_path, output_path, output_img_path):
    ...
    with torch.no_grad():
        ...
        for i in range(len(images) - 1):
            ...
            # Save flow
            np.savez(os.path.join(output_path, '%05d_fwd.npz'%i), flow=flow_fwd, mask=mask_fwd)
            np.savez(os.path.join(output_path, '%05d_bwd.npz'%(i + 1)), flow=flow_bwd, mask=mask_bwd)

            # Save flow_img
            Image.fromarray(flow_viz.flow_to_image(flow_fwd)).save(os.path.join(output_img_path, '%05d_fwd.png'%i))
            Image.fromarray(flow_viz.flow_to_image(flow_bwd)).save(os.path.join(output_img_path, '%05d_bwd.png'%(i + 1)))

            Image.fromarray(mask_fwd).save(os.path.join(output_img_path, '%05d_fwd_mask.png'%i))
            Image.fromarray(mask_bwd).save(os.path.join(output_img_path, '%05d_bwd_mask.png'%(i + 1)))
